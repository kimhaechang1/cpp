...
5. **Cycle of Practice (학습 주기)**
   ...
   - **Blank Slate Policy (선언부 및 타입 추론 훈련)**: 핵심 로직이 담긴 함수나 클래스 멤버/변수는 **선언부와 구체적인 타입**을 절대 직접 제공하지 않습니다. 주석으로는 오직 **"어떤 용도로 쓰일 것인지(목적)"** 와 **"변수/함수명"** 만 제시하여, 학습자가 상황에 맞는 적절한 타입과 함수 시그니처를 스스로 고민하고 선언하도록 유도해야 합니다. 단, 학습자가 편의상 `std::vector` 등을 사용하여 포인터 및 수동 메모리 관리 훈련 목적이 훼손되는 것을 방지하기 위해, **"동적 할당 변수"** 라는 목적성 키워드는 주석에 명시적으로 허용합니다. (직접적인 타입 힌트는 여전히 금지) **[🚨 강력/명시적 금지사항] 특히, 누적 문제(Cumulative Practice)나 미니 프로젝트에서는 "절대로, 그 어떤 형태의 주석 힌트(작성 요령, 문법 힌트, 팁 등)도 제공하지 마십시오." 실전처럼 오직 '요구되는 비즈니스 로직(목적)' 만을 서술해야 합니다.**
