...
## 5. const 참조 (const Reference) ⭐
함수에서 구조체나 큰 데이터를 받을 때 `&`(참조)를 쓰면 복사 비용이 없어서 빠릅니다.
하지만 **"원본이 수정될 위험"**이 있습니다.
이때 `const` 키워드를 붙이면 **"빠르면서도 안전하게"** 데이터를 읽을 수 있습니다.

### 5.1. const & 의 문법
```cpp
// void PrintStats(Stats& s) -> 원본 수정 가능 (위험)
// void PrintStats(Stats s)  -> 복사 발생 (느림)

// [BEST] 읽기 전용으로 빠르게 받기
// "s의 주소를 가져오되, s를 수정하지는 않겠다"는 약속입니다.
void PrintStats(const Stats& s) {
    // s.hp = 0; // ❌ 컴파일 에러! (수정 불가)
    cout << s.name << endl; // ⭕ 읽기는 가능
}
```

### 5.2. 언제 써야 할까요?
1. **수정할 필요가 없는** 큰 데이터(구조체, 클래스, 긴 string 등)를 받을 때
2. `Print...`, `Is...` , `Get...` 처럼 상태를 **확인만 하는 함수**일 때

> **Tip**: `int`, `enum` 처럼 작은 데이터는 그냥 값으로 받아도(`Call by Value`) 됩니다. `const &`는 덩치 큰 녀석들에게 효과적입니다.

---

## 6. 오버로딩 주의사항 (const 유무)

사용자분들이 자주 헷갈리는 부분입니다!
**"값(Value)이냐, 참조(Reference)냐"**에 따라 `const` 오버로딩 규칙이 다릅니다.

### 6.1. 일반 변수 (Call by Value) -> ❌ 오버로딩 안됨
```cpp
void Func(int a)       {}
void Func(const int a) {} // 에러! 같은 함수로 취급됨 (Redeclaration)
```
외부에서 볼 때 어차피 "복사"해서 넘겨주는 값이라, 내부에서 상수로 쓰든 말든 호출자는 알 바가 아니기 때문입니다.

### 6.2. 참조 변수 (Call by Reference) -> ✅ 오버로딩 됨
```cpp
void Func(int& a)       {} // "수정 가능한 원본 주세요"
void Func(const int& a) {} // "읽기 전용 원본 주세요" (상수도 가능)
```
이 경우는 명확히 다릅니다. 
- `Func(x)` (변수) -> `int&` 버전 우선 호출
- `Func(10)` (상수) -> `const int&` 버전 호출 (상수는 수정 불가능하므로)
