...
//     ProtocolType::UDP);
//     EXPECT_EQ(manager.RouteMessage(MessageType::LootReport),
//     ProtocolType::TCP);
// }
...
//     // 체력이 50 초과인 세션 필터링
//     auto healthyIds = manager.FilterSessions([](int id, int hp) {
//         return hp > 50;
//     });
...
//         if (id == 1) hasAlice = true;
//         if (id == 3) hasCharlie = true;
//     }
...
//     // 송신 (자신에게 보내기)
//     int sendResult = sendto(udpSock, reinterpret_cast<const
//     char*>(packetPtr->data()),
//                             static_cast<int>(packetPtr->size()), 0,
...
//     int recvResult = recvfrom(udpSock,
...
//     auto recoveredOpt = PositionMessage::Deserialize(payload);
