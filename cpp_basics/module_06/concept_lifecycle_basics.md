...
### 6.2 소멸자도 `private`이 되나요? (소멸의 통제)
**네, 이 역시 가능합니다!** 
소멸자를 `private`으로 닫아버리면, `main` 함수에서 `Slime s1;` 처럼 선언할 때 **컴파일 에러**가 발생합니다. 컴파일러가 "나중에 함수가 끝날 때 자동으로 지워줘야 하는데 권한이 없잖아? 그럼 처음부터 생성도 안 돼!" 라고 막아버리기 때문이죠.

*   **용도**:
    *   **"이 객체는 절대로 지역 변수(Stack)로 만들어서 자동으로 증발하게 놔두지 말고, 무조건 `new`(동적 할당, Heap)로만 만들어서 평생 끈질기게 살아남아 관리되게 해라!"** 하고 다른 프로그래머들에게 설계를 강제하고 싶을 때 쓰이는 현업 엔진의 고급 패턴입니다.

### 6.3 [Q&A] 파이썬처럼 이름으로 값 넣기 (Kwargs)?
...
## 7. 심화: 임시 객체와 반환값 최적화 (RVO vs NRVO)
객체를 생성하고 반환할 때, 우리는 두 가지 방식을 선택할 수 있습니다. 이 둘은 메모리 생명주기와 컴파일러 최적화 관점에서 완전히 다릅니다.

### 7.1 이름 있는 지역 변수 (`Player p;`)
```cpp
Player p; 
```
*   **생성자 호출**: 즉시 기본 생성자가 호출되어 메모리가 셋팅됩니다.
*   **수명 (Lifetime)**: 해당 변수가 선언된 `{ }` 블록이 끝날 때까지 끈질기게 살아남습니다.
*   **반환 시 특징 (NRVO)**: `return p;` 형태로 반환하면 컴파일러는 고민에 빠집니다. 원칙상 복사 생성자를 호출해 복사본을 만들어 던져야 하지만, 상황표 따라 최적화(NRVO)를 해줄 수도 있고, 끄면(디버그 모드) 안 해줄 수도 있습니다.

### 7.2 허공의 임시 객체 (`Player();`)
```cpp
Player(); 
```
*   **생성자 호출**: 호출됨과 동시에 형태를 갖춥니다.
*   **수명 (Lifetime)**: 이름이 없는 유령 객체입니다. 오직 저 코드가 적힌 **"그 줄(세미콜론 `;` 지점)"** 까지만 생존하고 즉시 소멸자가 불리며 파괴됩니다.
*   **반환 시 특징 (PRVO)**: `return Player();` 형태로 반환하면, C++17 표준에 의해 **"무조건 복사를 생략하고(Copy Elision), 받는 쪽의 주소에 다이렉트로 생성하라"**는 판정이 내려집니다. 최상의 성능을 냅니다.

### 7.3 🚨 C++ 최고 악명 높은 문법 낚시 (Most Vexing Parse)
기본 생성자를 호출할 때 괄호(`()`)를 붙이면 어떻게 될까요?
```cpp
// 1. 정상적인 객체 생성 (기본 생성자 호출)
Player p1;

// 2. 🚨 함수 선언으로 오해받음 (객체 안 만들어짐!)
Player p2(); 
```
C++ 컴파일러는 `Player p2();`를 보는 순간 객체를 만드는 것이 아니라, **"아, 반환형이 Player고 매개변수가 없는 p2라는 이름의 함수를 선언했구나"**라고 단단히 착각합니다.
따라서, 매개변수가 없는 객체를 선언할 때는 **절대 괄호를 붙이지 않거나**, 모던 C++의 방식인 `Player p3{};` (유니폼 초기화)를 사용해야 가장 안전합니다!

---

### 🚨 [Advanced] 멤버 초기화 리스트의 실무 원칙 (M10 심화)

**1. 대입보다 초기화**: 
클래스의 몸체(`{ }`)에서 대입하는 것은 "이미 생성된 객체에 값을 덮어쓰는 것"이고, 초기화 리스트(`:`)는 "객체가 태어날 때 그릇에 바로 값을 담는 것"입니다. 효율이 압도적입니다.

**2. 변수 선언 순서대로**: 
초기화 리스트에 적은 순서가 아니라, **클래스 선언부에 적힌 변수의 선언 순서대로** 초기화됩니다. 의외의 버그를 막기 위해 초기화 리스트도 선언 순서와 맞춰 적어주는 것이 관례입니다.

**3. 상속 구조에서의 위임**: 
부모 클래스의 멤버는 자식이 직접 초기화하지 말고, **부모의 생성자를 호출**하여 초기화 리스트에서 처리하세요.
