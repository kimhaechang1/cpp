...
//    - 외부 노출은 금지하되 자식에게만 허용되는 멤버 변수(정수형 itemId)를
//    가지세요.
...
public:
  ...
  Item(Item &&other) { this->itemId = other.itemId; }
...
public:
  ...
  std::string GetItemName() override { return this->name; }

  Weapon(Weapon &&other) : Item{other.itemId} { this->name = other.name; }
...
// 내부에 아이템의 고유 ID를 Key로 하고, "아이템의 독점 소유권 래퍼"를 Value로
// 가지는 map(이름: itemTable) 구조를 하나 선언합니다.
// -> 여기에 작성
struct DbManager {
...
// 2. 반환값으로 "아이템 독점 소유권의 무리의 1차원 컨테이너(가변 가변 배열)"를
// 리턴합니다.
// 3. (구조 분해 할당) 원본 DbManager의 itemTable 맵을 순회하면서, 직관적인
// 이름으로 분해하세요. (ex. id, item)
// 4. (이동 시맨틱) 분해한 후 원본 맵에 있는 아이템 소유권을 강제로
// 빼앗아(강탈), 반환할 배열에 밀어 넣습니다.
//    참고로, 원본 맵은 더 이상 데이터를 가지고 있지 않아야 정상 작동한
//    것입니다!
// -> 여기에 작성
...
int main() {
  ...
  // TODO 3에서 만든 함수를 호출하여 무기 배열 획득 (반환된 객체를 'auto' 로
  // 받습니다)
  // -> 여기에 호출 코드 작성 (auto extractedItems = ...)
  auto extractedItems = ExtractAllWeapons(manager);

  // 검증 2: 원본 맵에서 데이터가 완전히 뽑아져 나갔는지 검증!
  // (이동 처리가 잘 되었다면 원본 객체 내부 소유권 포인터들은 nullptr 가 되지만
  // map 원소 갯수가 지워지진 않음. 하지만 만약 요소 내부가 비었는지
  // 체크하는거라면, 추출된 갯수로 테스트합니다.)
  ...
  // -> 추출된 리스트의 첫 번째 아이템의 타입을 완벽히 복제한 변수
  // 'firstItemClone(임시 포인터)'를 작성하세요.
  // -> 값의 복사가 아니라 동일한 소유권 래퍼의 "타입"만 가져와서,
  // 이동(move)으로 받아와야 합니다.
  // -> 여기에 작성
  ...
  std::cout
      << "🌟 9-2 종합 테스트 (다형성, 소유권 이동, 구조 분해) 통과 완료! 🌟\n";
  return 0;
}
