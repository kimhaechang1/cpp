...
**Q3. `release()`와 `get()`의 결정적인 차이와 위험성은 무엇인가요?**
*   **A**: **소유권(Ownership)의 포기 여부**가 핵심입니다.
    *   **`get()` (주소 대여)**: "계약서는 유지할게. 주소만 잠깐 보여줄게." 포인터만 반환하고 `unique_ptr`은 여전히 메모리를 책임집니다. 매우 안전한 방식입니다.
    *   **`release()` (소유권 유기)**: "이제 내 물건 아냐! 니가 직접 관리해!" 메모리는 해제하지 않은 채 관리 책임자(`unique_ptr`)만 사라집니다. 반환된 생포인터를 즉시 다른 관리자에게 넘기거나 직접 `delete` 하지 않으면 **메모리 릭**이 발생하며, 비어버린(`nullptr`) 기존 `unique_ptr`를 다시 쓰려 하면 **크래시**가 납니다. 보통은 C API와의 인터페이스 등 아주 특수한 경우에만 사용합니다.
