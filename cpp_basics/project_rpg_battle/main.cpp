...
class IAttacker {
public:
    virtual void Attack(BaseUnit* target) = 0;
};
...
class IHealer {
public:
    virtual void Heal(BaseUnit* target) = 0;
};
...
protected:
    ...
    Element element;
public:
    BaseUnit(std::string _name, int _hp, Element _elmemnt, Position _pos): name(_name), hp(_hp), maxHp(_hp), pos(pos) {
...
class Warrior : public BaseUnit, public IAttacker {
public:
    Warrior(std::string _name, int _hp, Element _elem, Position _pos) : BaseUnit{_name, _hp, _elem, _pos} {
    ...
    void Action() override{
        std::cout << "[" << this->name << "] readies sword." << std::endl;
    }
    void Attack(BaseUnit* target) override {
...
class Cleric : public BaseUnit, public IHealer {
public:
    Cleric(std::string _name, int _hp, Element _elem, Position _pos) : BaseUnit{_name, _hp, _elem, _pos} {
    ...
    void Heal(BaseUnit* target) override {
        target->TakeDamage(-20);
        std::cout << "[" << this->name << "] heals [" << target->GetName() << "]!" << std::endl;
    }
};
...
public:
    ...
    std::span<BaseUnit*> GetUnits() {
        return std::span(this->units);
    }
    void SimulateCombat() {
        int alliveId = -1;
        for (int i = 0; i < this->units.size(); i++) {
            ...
            IAttacker* attacker = dynamic_cast<IAttacker*>(units[i]);
            if (attacker == nullptr) continue;
            if (alliveId == i) continue;
            if (alliveId == -1) continue;
            attacker->Attack(units[alliveId]);
        }
    }
};
